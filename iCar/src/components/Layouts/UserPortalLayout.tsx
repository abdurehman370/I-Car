...
const navItems = [
  { title: "Auctions", url: "/user/auctions", icon: Gavel },
  { title: "Auction Alerts", url: "/user/auction-notifications", icon: BellRing },
...
export default function UserPortalLayout({ children }: Props) {
  ...
  const headerLabel =
    pathname.startsWith("/user/auction-notifications")
      ? "Auction Alerts"
      ...
      >
        ...
        <div className="px-3 py-2">
          ...
          <nav className="space-y-1.5">
            {navItems.map((item) => {
              const active =
                item.url === "/user/auctions"
                  ? pathname === "/user/auctions" ||
                    pathname.startsWith("/user/auctions/")
              ...
              return (
                ...
                >
                  {active && (
                    <span
                      className="absolute top-1/2 -translate-y-1/2 h-8 w-[3px] rounded-full bg-violet-400 shadow-[0_0_15px_#a78bfa]"
                      style={{ left: -1 }}
                    />
                  )}
                  <item.icon
      ...
      >
        <header className="sticky top-0 z-30 h-16 flex items-center gap-4 px-4 md:px-8 glass-strong border-b border-white/5">
          ...
          <div className="hidden md:flex items-center gap-2 relative ml-4">
            <div className="absolute -left-3 top-1/2 -translate-y-1/2 h-6 w-[2px] bg-violet-400 shadow-[0_0_10px_#a78bfa] rounded-full" />
            <span className="font-mono text-[10px] tracking-[0.3em] text-muted-foreground uppercase opacity-60 ml-2">
              User Portal · {headerLabel}
            </span>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <Link
              href="/user/auction-notifications"
              className="relative h-10 w-10 rounded-xl flex items-center justify-center text-gray-400 hover:text-white hover:bg-white/5 transition-colors"
            >
              <BellRing className="h-5 w-5" />
            </Link>

            <div className="h-10 w-10 rounded-xl bg-gradient-to-br from-violet-400/20 to-cyan-500/20 border border-violet-400/30 flex items-center justify-center shrink-0">
              <span className="text-sm font-bold text-violet-300">U</span>
            </div>
          </div>
        </header>
