    ...
    <div className="space-y-8 max-w-[1600px] mx-auto p-4 md:p-0">
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
        ...
        <div className="md:col-span-4 flex gap-4">
          <div className="flex-1 relative">
             <Filter className="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-gray-500" />
              <select 
                ...
                className="w-full h-14 pl-11 pr-4 rounded-2xl glass border border-white/10 text-foreground appearance-none outline-none focus:border-cyan-400/30"
              >
          ...
          <button className="h-14 w-14 rounded-2xl glass border border-white/10 flex items-center justify-center text-gray-500 hover:text-foreground transition-colors">
    ...
    >
      <div className="relative aspect-[16/10] overflow-hidden bg-white/5">
        ...
        <div className="absolute bottom-3 left-4 right-4">
            <h4 className="font-bold text-[#ffffff] text-lg tracking-tight truncate">{v.make} {v.model}</h4>
      ...
      <div className="p-5 flex-1 flex flex-col justify-between space-y-4">
        <div>
            <div className="flex items-baseline justify-between mb-4">
                ...
                <span className="text-[10px] font-mono text-cyan-500/70">AED {(v.price / 3.67).toFixed(0)} USD equivalent</span>
            </div>
            ...
            <div className="grid grid-cols-3 gap-2">
                ...
                <div className="flex flex-col items-center gap-1 p-2 rounded-xl bg-white/5 border border-white/5 group-hover:border-white/10 transition-colors">
                    ...
                    <span className="text-[10px] font-bold text-foreground/70">{(v.mileage / 1000).toFixed(0)}K KM</span>
                </div>
                <div className="flex flex-col items-center gap-1 p-2 rounded-xl bg-white/5 border border-white/5 group-hover:border-white/10 transition-colors">
                    <MapPin className="h-3.5 w-3.5 text-cyan-500" />
                    <span className="text-[10px] font-bold text-foreground/70 truncate w-full text-center">{v.city}</span>
                </div>
            </div>
        </div>
        ...
        <div className="flex items-center gap-2 pt-2">
            <Link href={`/listings/${v.id}`} className="flex-1">
                <button className="w-full h-10 rounded-xl glass border border-white/10 text-foreground text-xs font-bold flex items-center justify-center gap-2 hover:bg-white/10 transition-all group/btn">
                    <Eye className="h-3.5 w-3.5 group-hover/btn:scale-110 transition-transform" /> View
                </button>
            </Link>
            <button className="h-10 w-10 rounded-xl glass border border-white/10 text-muted-foreground hover:text-cyan-500 flex items-center justify-center transition-all">
